...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.btn_merge = QPushButton("▶ Ghép nối hoàn chỉnh")
        ...
        self.btn_clean = QPushButton("Dọn Rác (.txt)")
        ...
        if success_count > 0:
            ...
            try:
                if os.path.exists("success.wav"):
                    # Phát âm thanh ngầm (SND_ASYNC) để không làm treo giao diện
                    winsound.PlaySound("success.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)
            except Exception as e:
                self.log(f"[-] Không thể phát âm thanh: {e}")
                
        else:
            self.lbl_drop_txt_merge.setStyleSheet("QLabel { border: 2px solid #F44336; border-radius: 5px; padding: 20px; background-color: #FFEBEE; font-size: 14px; color: #C62828; font-weight: bold; }")
            self.lbl_drop_txt_merge.setText(f"❌ Dịch thất bại toàn bộ! Hãy kiểm tra lỗi.")
