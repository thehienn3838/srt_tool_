...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SRT Tool Pro v2.1 - Tự Động Hóa 100%")
        ...
        # ==========================================
        # CỘT BÊN TRÁI: CÁC CÔNG CỤ THAO TÁC
        # ==========================================
        left_widget = QWidget()
        ...
        # 1. Trích xuất
        group_extract = QGroupBox("1. Trích Xuất Text (Kéo file/thư mục vào đây)")
        layout_extract = QVBoxLayout()
        self.lbl_drop_srt = DropLabel("Kéo thả File hoặc Thư mục chứa SRT vào đây")
        self.btn_extract = QPushButton("▶ Trích xuất hàng loạt")
        ...
        # 2. Bối cảnh (Đã thu nhỏ cố định)
        group_context = QGroupBox("🧠 Từ Điển / Bối Cảnh (Sẽ chèn vào AI)")
        layout_context = QVBoxLayout()
        layout_context.setContentsMargins(10, 15, 10, 10) # <--- Bóp nhỏ khoảng cách lề bên trong
        ...
        group_context.setFixedHeight(95) # <--- ÉP CỨNG CHIỀU CAO CỦA CẢ KHUNG VIỀN NGOÀI
        left_layout.addWidget(group_context)

        # 3. Trạng thái hệ thống
        group_status = QGroupBox("2. Trạng Thái Hệ Thống (Auto)")
        layout_status = QVBoxLayout()
        self.lbl_status = QLabel("Chưa có tiến trình nào đang chạy.")
        ...
        # 4. NÚT TẠO LỆNH (ĐÃ DI CHUYỂN VỀ ĐÂY)
        self.btn_auto_cmd = QPushButton("⚡ TẠO LỆNH DỊCH TỰ ĐỘNG")
        self.btn_auto_cmd.setMinimumHeight(45)
        self.btn_auto_cmd.setStyleSheet("background-color: #ff9800; color: white; font-weight: bold; font-size: 14px; border-radius: 5px;")
        self.btn_auto_cmd.clicked.connect(self.generate_auto_command)
        left_layout.addWidget(self.btn_auto_cmd)
        ...
        # ==========================================
        # CỘT BÊN PHẢI (CHỈ CÒN ĐÚNG MÀN HÌNH TERMINAL)
        # ==========================================
        ...
        term_header_layout.addWidget(QLabel("Terminal (Chạy ngầm)"))
    ...
    def generate_auto_command(self):
        ...
        self.lbl_status.setText(f"⏳ Đã tạo kịch bản. Vui lòng ấn Enter ở Terminal (Cột phải)")
        self.lbl_status.setStyleSheet("QLabel { border: 2px dashed #aaa; border-radius: 5px; padding: 20px; background-color: #f9f9f9; font-size: 15px; color: #333333; font-weight: bold; }")

        cmd = f'@call "{bat_path}"'
        self.term_input.setText(cmd)
        self.log(f"[*] Đã đóng gói kịch bản. Ấn Enter ở Terminal để chạy 1 lèo!")
    ...
    def auto_merge(self):
        ...
        for srt_path in self.target_files:
            ...
            try:
                ...
                self.log(f"[!] Lỗi khi tự động ghép {os.path.basename(srt_path)}: {e}")
    ...
    def auto_cleanup(self):
        ...
        self.log(f"[🧹] Tự động dọn dẹp hoàn tất! Đã xóa sạch {deleted_count} file rác.")

    def start_translation_ui(self):
        self.is_translating = True
        self.output_buffer = ""
        self.spinner_idx = 0
        self.lbl_status.setStyleSheet("QLabel { border: 2px dashed #FF9800; border-radius: 5px; padding: 20px; background-color: #FFF3E0; font-size: 15px; color: #E65100; font-weight: bold; }")
        self.spinner_timer.start(100)

    def update_spinner(self):
        char = self.spinner_chars[self.spinner_idx]
        self.lbl_status.setText(f"{char} AI Đang Dịch ({len(self.target_files)} file)...")
    ...
    def finish_translation_ui(self):
        ...
        if success_count > 0:
            self.log(f"[*] AI đã dịch xong {success_count} file. Bắt đầu xử lý hậu kỳ tự động...")
            
            self.lbl_status.setText(f"🔄 Đang tự động ghép nối...")
            QApplication.processEvents() 
            merged_count = self.auto_merge()
            
            self.lbl_status.setText(f"🧹 Đang dọn dẹp file rác...")
            QApplication.processEvents()
            self.auto_cleanup()

            self.lbl_status.setStyleSheet("QLabel { border: 2px solid #4CAF50; border-radius: 5px; padding: 20px; background-color: #e8f5e9; font-size: 16px; color: #2E7D32; font-weight: bold; }")
            self.lbl_status.setText(f"✅ HOÀN TẤT 100%!\nĐã lưu {merged_count} video Vietsub.")
            ...
            self.lbl_status.setStyleSheet("QLabel { border: 2px solid #F44336; border-radius: 5px; padding: 20px; background-color: #FFEBEE; font-size: 15px; color: #C62828; font-weight: bold; }")
            self.lbl_status.setText(f"❌ Dịch thất bại toàn bộ! Hãy kiểm tra lỗi ở Terminal.")
